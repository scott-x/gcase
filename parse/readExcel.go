...
func Write_Data() (s1, s2 string) {
  project_folder := utils.Dir() //返回的是main方法的目录
  fmt.Printf("project_folder: %s", project_folder)
  from := path.Join(project_folder, "xlsx_templates/WMT_xxxxxx_xxx 做稿")
  to := path.Join(project_folder, "temp/WMT_xxxxxx_xxx 做稿")
  ...
  fmt.Println(job)
  // //rename file
  oldFile := project_folder + "/temp/WMT_xxxxxx_xxx 做稿/U180XXX_XXX_DetailList_W.xlsx"
  newFile := project_folder + "/temp/WMT_xxxxxx_xxx 做稿/" + strings.ToUpper(job.job_number) + "_DetailList_W.xlsx"
  ...
  //rename folder
  oldFolder := project_folder + "/temp/WMT_xxxxxx_xxx 做稿"
